...
export function DashboardPage() {
  ...
  useEffect(() => {
    void api("/api/stats/weekday-radar").then((r) => setRadar(r.weekdays));
  ...
  const periodMinutes = (() => {
    switch (period) {
      case "day": return totals.todayMinutes ?? 0;
      case "week": return totals.weekMinutes ?? 0;
      case "month": return totals.monthMinutes ?? 0;
      case "total": return totals.totalMinutes ?? 0;
      default: return 0;
    }
  })();
  ...
  const periodSessions = (() => {
    switch (period) {
      case "day": return totals.todaySessionCount ?? 0;
      case "week": return totals.weekSessionCount ?? 0;
      case "month": return totals.monthSessionCount ?? 0;
      case "total": return totals.sessionCount ?? 0;
      default: return 0;
    }
  })();
  ...
  const periodGoalMinutes = (() => {
    ...
    switch (period) {
      ...
      case "week": return daily * 7;
      case "month": {
        const d = new Date();
        const dim = new Date(d.getFullYear(), d.getMonth() + 1, 0).getDate();
        return daily * dim;
      }
      case "total": return (term?.studyGoalHours ?? 600) * 60;
          ...
          <div className="space-y-4">
            ...
            <div className="rounded-2xl border border-slate-800 bg-slate-900/50 p-4">
              ...
              <div className="mt-3 flex gap-4 text-[0.9375rem]">
                <span className="text-amber-300">Gold {stats?.term?.goldMedals ?? 0}</span>
                <span className="text-slate-300">Silver {stats?.term?.silverMedals ?? 0}</span>
                <span className="text-amber-700/90">Bronze {stats?.term?.bronzeMedals ?? 0}</span>
              </div>
            </div>
