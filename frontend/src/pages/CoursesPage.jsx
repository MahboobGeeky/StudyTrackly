...
export function CoursesPage() {
  ...
  const load = useCallback(async () => {
    ...
    setTerm(t);
    if (!t) return;
    const cs = await api(`/api/courses?termId=${t.id}`);
    setCourses(cs);
  }, []);
  ...
  return (
    <>
      ...
      <main className="flex-1 space-y-6 overflow-auto p-6">
              ...
              {editingId === c.id ? (
                <form onSubmit={saveEdit} className="space-y-3">
                  ...
                  <div className="flex gap-2">
                    <button type="submit" className="rounded-lg bg-blue-600 px-3 py-1.5 text-sm text-white">
                      Save
                    </button>
                    <button
                      type="button"
                ...
                <div className="flex items-center justify-between gap-2">
                  ...
                  <div className="flex gap-2">
                    <button type="button" onClick={() => startEdit(c)} className="text-xs text-blue-400 hover:underline">
                      Edit
                    </button>
                    <button type="button" onClick={() => void remove(c.id)} className="text-xs text-red-400 hover:underline">
                      Remove
                    </button>
                  </div>
                </div>
        ...
        {!term && (
          <p className="text-sm text-slate-500">No active term. Create one under Term Configuration.</p>
        )}
      </main>
    </>
  );
