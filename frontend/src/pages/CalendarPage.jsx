...
export function CalendarPage() {
  ...
  const load = useCallback(async () => {
    ...
    const [ss, sd] = await Promise.all([
      api(`/api/sessions?termId=${t.id}`),
      api(`/api/study-days?termId=${t.id}`),
    ]);
    setSessions(ss);
            ...
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div className="flex flex-wrap gap-2 text-[0.8125rem]">
                {["Days", "Weeks", "Months"].map((t, i) => (
                  <span key={t} className={i === 0 ? "rounded-full bg-slate-800 px-3 py-1.5 text-white" : "text-slate-500"}>
                    {t}
                  </span>
                ))}
              </div>
          ...
          <aside className="flex w-full flex-col gap-4 xl:max-w-sm xl:justify-self-end">
            <div className="rounded-2xl border border-slate-800 bg-slate-900/50 p-4">
              <p className="text-[0.8125rem] text-slate-500">
                {sel ? `${format(new Date(sel.date), "dd/MM/yy")} • ${format(new Date(sel.date), "EEEE")}` : "Select a row"}
              </p>
              <p className="mt-2 text-[0.9375rem] font-medium text-slate-100">Sessions • {daySessions.length}</p>
              <ul className="mt-3 space-y-3">
                {daySessions.map((s) => {
                  const mins = sessionDurationMinutes(s.startTime, s.endTime, s.breakMinutes);
                  return (
                    <li key={s.id} className="rounded-lg border border-slate-800 bg-slate-950/60 p-3 text-[0.8125rem]" onClick={() => setSelected(s.id)}>
                      <div className="flex items-center justify-between gap-2">
                        <span className="text-xs text-orange-300">{s.course?.name ?? "Unknown"}</span>
